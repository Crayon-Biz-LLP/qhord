...
const subscription_service_disabled_1 = require("../services/subscription-service.disabled");
...
router.get('/status', async (req, res) => {
    try {
        ...
        const subscription = await subscription_service_disabled_1.SubscriptionService.getUserCredits(userId);
        res.json({
            success: true,
            subscription
        });
...
router.get('/credits', async (req, res) => {
    try {
        const userId = req.user.id;
        const credits = await subscription_service_disabled_1.SubscriptionService.getUserCredits(userId);
        res.json({
            success: true,
            credits
        });
...
router.post('/check-tool-access', async (req, res) => {
    try {
        ...
        const access = await subscription_service_disabled_1.SubscriptionService.checkToolAccess(userId, toolName);
        res.json({
            success: true,
            access
        });
...
router.get('/usage-history', async (req, res) => {
    try {
        ...
        const history = await subscription_service_disabled_1.SubscriptionService.trackUsage(userId, 0);
        res.json({
            success: true,
            history
        });
...
router.get('/plans', async (req, res) => {
    try {
        const plans = tool_registry_1.ToolRegistry.getAllTools();
        const subscriptionPlans = tool_registry_1.ToolRegistry.getSubscriptionPlan('free');
        res.json({
            success: true,
            plans: [
                {
...
router.post('/consume-credits', async (req, res) => {
    try {
        ...
        const success = await subscription_service_disabled_1.SubscriptionService.trackUsage(userId, -credits);
