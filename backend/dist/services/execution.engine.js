...
class ExecutionEngine {
    async execute(request, operatorId) {
        ...
        const accountResult = await (0, db_1.query)('SELECT * FROM client_tool_accounts WHERE id = $1 AND client_id = $2', [toolAccountId, clientId]);
        const account = accountResult.rows[0];
        if (!account) {
            throw new Error('Tool account not found for client');
        }
        const apiKey = (0, encryption_1.decrypt)(account.api_key_encrypted);
        const executionInsert = await (0, db_1.query)(`INSERT INTO executions (
        ...
        const updated = await (0, db_1.query)('UPDATE executions SET status = $1, response_payload = $2, error_message = $3, updated_at = NOW() WHERE id = $4 RETURNING *', [status, responsePayload, errorMessage, execution.id]);
