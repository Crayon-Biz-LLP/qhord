...
import {
   ...
   Moon, Sun, Wand2, ArrowRight, Bell, CheckCircle, XCircle
} from "lucide-react";
import { ApprovalDashboard } from "../../approvals/ApprovalDashboard";
import { CampaignReviewModal } from "../../campaigns/CampaignReviewModal";
...
export const Workflows = ({ onBackToDashboard }: WorkflowsProps) => {
   ...
   const fetchWorkflows = async () => {
      try {
         ...
         const mapped: WorkflowItem[] = (data.workflows || []).map((wf: any) => ({
            id: wf.id,
            name: wf.name,
            status: "draft",
            metadata: [
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden bg-[#f7f8f9] text-[#1a1510] selection:bg-brand-gold/30 font-sans">
         ...
         <main className="flex-1 overflow-y-auto p-6 lg:p-8 space-y-6 scrollbar-hide pb-32">
            ...
            <div className="flex items-end justify-between">
               ...
               >
                  <div>
                     <h1 className="text-xl font-extrabold tracking-tight text-[#1a1510] mb-1 leading-none">
                        {activeTab === "workflows" ? "Workflows" : "Campaign Approvals"}
                     </h1>
                     <p className="text-xs font-medium text-[#1a1510]/40">
                        {activeTab === "workflows" 
                           ? "GTM Engine — Your revenue automation is running at <span className='text-emerald-500 font-bold'>94% health</span>"
                           : "Review and approve pending campaigns for execution"
                        }
                     </p>
                  </div>
                  
                  {/* Tab Navigation */}
                  <div className="flex bg-[#f7f8f9] rounded-lg p-1">
                     <button
