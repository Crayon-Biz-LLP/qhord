    ...
    <div className="flex-1 flex flex-col h-full bg-[#f7f8f9] text-[#1a1510] font-sans selection:bg-[#D4AF37]/30">
      ...
      <header className="h-16 border-b border-[#1a1510]/5 bg-white flex items-center justify-between px-4 sm:px-8 shrink-0 z-50 shadow-sm relative">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:block truncate">
              <div className="flex items-center gap-2">
                <h1 className="text-sm font-bold tracking-tight text-[#1a1510] uppercase truncate">Command Centre</h1>
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <button className="h-10 px-4 sm:px-6 rounded-xl bg-[#1a1510] text-[#D4AF37] text-[10px] font-bold uppercase tracking-widest flex items-center gap-2 shadow-xl hover:translate-y-[-1px] transition-all group shrink-0">
      ...
      <main className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 scrollbar-hide">
        ...
        <section className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3 sm:gap-4">
          {DASHBOARD_DATA.kpis.map((kpi, i) => (
            <motion.div
              ...
              className={`bg-white p-3 rounded-2xl border border-[#1a1510]/5 flex flex-col justify-between h-24 sm:h-28 group transition-all shadow-sm hover:shadow-md cursor-default ${i > 3 ? 'hidden lg:flex' : 'flex'}`}
            >
              <div className="flex justify-between items-start">
                <span className="text-[8px] font-bold text-[#1a1510]/30 tracking-widest uppercase">{kpi.label}</span>
              ...
              <div>
                <h3 className="text-lg sm:text-xl font-bold text-[#1a1510] tracking-tighter mb-0.5">{kpi.value}</h3>
        ...
        {/* Action Bar */}
        <section className="flex flex-wrap items-center gap-2 sm:gap-3 py-2 bg-[#f7f8f9]/50 p-2 sm:p-3 rounded-[1.5rem] sm:rounded-3xl border border-[#1a1510]/5 overflow-x-auto scrollbar-hide">
          {[
            ...
            <button
              key={i}
              className="h-9 px-4 rounded-lg border border-[#1a1510]/5 bg-white text-[9px] font-bold uppercase tracking-widest text-[#1a1510]/50 hover:text-[#1a1510] transition-all flex items-center gap-2 whitespace-nowrap"
            >
          ...
          <button
            ...
            className="h-9 px-6 rounded-lg bg-brand-gold text-[#1a1510] text-[9px] font-bold uppercase tracking-widest flex items-center gap-2 shadow-md hover:translate-y-[-1px] transition-all shrink-0"
          >
        ...
        <section className="relative">
          ...
          >
            <div className="p-5 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12">
              <div className="flex flex-col md:flex-row items-center gap-6 sm:gap-8 text-center md:text-left">
                <div className="relative group shrink-0">
                  ...
                  <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl sm:rounded-3xl bg-[#1a1510] border border-brand-gold/20 flex items-center justify-center text-brand-gold relative z-10 transition-transform hover:scale-105">
                    <Bot size={32} />
                  </div>
                </div>
                ...
                <div className="space-y-1.5 min-w-0">
                  <div className="flex flex-wrap items-center gap-3 justify-center md:justify-start">
                    <h2 className="text-xl sm:text-2xl font-bold text-[#1a1510] tracking-tighter">AI Operator</h2>
                  ...
                  <p className="text-[10px] sm:text-xs font-semibold text-[#1a1510]/40 max-w-sm leading-relaxed uppercase tracking-widest truncate sm:whitespace-normal">
                    Monitoring egress layers — optimized across <span className="text-brand-gold font-bold">4 active vectors.</span>
                  </p>
                </div>
              </div>
              ...
              <div className="bg-[#f7f8f9] p-1.5 rounded-xl sm:rounded-2xl border border-[#1a1510]/5 flex items-center gap-1 shrink-0 overflow-x-auto">
                {(["Manual", "Assisted", "Autopilot"] as const).map((mode) => (
                  <button
                    ...
                    className={`px-4 sm:px-6 py-2 rounded-lg sm:rounded-xl text-[8px] sm:text-[10px] font-bold uppercase tracking-widest transition-all whitespace-nowrap ${operatorMode === mode
            ...
            <div className="border-t border-[#1a1510]/5 bg-[#fcfcfc] grid grid-cols-2 lg:grid-cols-4 divide-x divide-[#1a1510]/5">
              {[
                { label: "Protected", value: DASHBOARD_DATA.aiOperator.protectedRevenue, icon: ShieldCheck, color: "text-emerald-600", bg: "bg-emerald-50" },
                { label: "Unlocked", value: DASHBOARD_DATA.aiOperator.unlockedRevenue, icon: Sparkles, color: "text-brand-gold", bg: "bg-brand-gold/5" },
                { label: "Safe Actions", value: DASHBOARD_DATA.aiOperator.safeActions, icon: Zap, color: "text-blue-600", bg: "bg-blue-50" },
                { label: "Avoided", value: DASHBOARD_DATA.aiOperator.risksAvoided, icon: Target, color: "text-red-600", bg: "bg-red-50" },
              ].map((stat, i) => (
                <div key={i} className="p-3 sm:p-5 hover:bg-white transition-all group">
                  <div className="flex items-center gap-3 mb-2">
                    <div className={`p-1.5 sm:p-2 rounded-lg ${stat.bg} ${stat.color} shadow-sm shrink-0`}>
                      <stat.icon size={14} />
                    </div>
                    <span className="text-[8px] sm:text-[9px] font-bold uppercase tracking-widest text-[#1a1510]/20 hidden md:inline truncate">{stat.label}</span>
                  </div>
                  <div className="flex items-baseline gap-2">
                    <span className="text-lg sm:text-2xl font-bold text-[#1a1510] tracking-tighter truncate">{stat.value}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <section className="lg:col-span-8 space-y-6 order-2 lg:order-1">
            <div className="flex items-center justify-between px-2">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-xs font-bold text-[#1a1510] uppercase tracking-widest leading-none">Campaign Health</h3>
              </div>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full whitespace-nowrap text-left border-collapse">
                  <thead className="bg-[#fcfcfc] border-b border-[#1a1510]/5">
                    <tr>
                      <th className="py-3 px-6 text-[9px] font-bold text-[#1a1510]/20 uppercase tracking-widest">Name</th>
                      <th className="py-3 px-4 text-[9px] font-bold text-[#1a1510]/20 uppercase tracking-widest text-center">Status</th>
                      <th className="py-3 px-4 text-[9px] font-bold text-[#1a1510]/20 uppercase tracking-widest text-center">Health</th>
                      <th className="py-3 px-6 text-[9px] font-bold text-[#1a1510]/20 uppercase tracking-widest text-right">Pipeline</th>
                    </tr>
                  </thead>
                    ...
                    {DASHBOARD_DATA.healthTable.map((row, i) => (
                      <tr key={i} className="hover:bg-[#f7f8f9] transition-colors group cursor-pointer font-medium">
                        <td className="py-3 px-6">
                          <div className="flex items-center gap-4 min-w-0">
                            ...
                            <div className="truncate">
                              <p className="text-xs font-bold text-[#1a1510] group-hover:text-brand-gold transition-colors truncate">{row.name}</p>
                        ...
                        <td className="py-3 px-4 text-center">
                          <span className={`text-[8px] font-bold px-2 py-0.5 rounded-full uppercase tracking-widest ${row.status === 'Active' ? 'bg-emerald-50 text-emerald-600' : 'bg-[#1a1510]/5 text-[#1a1510]/30'
                            }`}>
                            {row.status}
                          </span>
                        </td>
                        <td className="py-3 px-4 text-center">
                          <div className="flex items-center justify-center gap-2">
                            ...
                            <span className="text-[9px] font-bold text-[#1a1510]/40">{row.health}</span>
                          </div>
                        </td>
                        <td className="py-3 px-6 text-right">
                          <div className="flex flex-col items-end">
                            <span className="text-xs font-bold text-[#1a1510]">{row.pipeline}</span>
          ...
          <section className="lg:col-span-4 space-y-8 order-1 lg:order-2">
            <div className="space-y-4 px-2 sm:px-0">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-xs font-bold text-[#1a1510] uppercase tracking-widest leading-none">Action Queue</h3>
              </div>
              ...
              <div className="space-y-4">
                {DASHBOARD_DATA.priorities.map((item, i) => (
                  <motion.div
                    key={i}
                    className="p-4 rounded-2xl bg-white border border-[#1a1510]/5 group hover:border-brand-gold/30 transition-all shadow-sm"
                  >
                    <div className="flex items-start justify-between mb-3">
                      <span className={`text-[8px] font-bold px-2 py-0.5 rounded uppercase tracking-widest ${item.bg} ${item.color}`}>{item.type}</span>
                      <span className="text-[9px] font-bold text-[#1a1510]/20 uppercase">{item.entity}</span>
                    </div>
                    <h4 className="text-sm font-bold text-[#1a1510] mb-1 leading-tight">{item.title}</h4>
